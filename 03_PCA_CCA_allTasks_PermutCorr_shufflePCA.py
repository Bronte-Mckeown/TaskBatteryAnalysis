...
#################################### Data #####################################
if not os.path.exists("scratch//results//cca//allTasks"):
    os.makedirs("scratch//results//cca//allTasks")
...
ca, X_c, Y_c, X_Y_sum, X_c_scaler, Y_c_scaler, corrs = train_CCA(
    pca_scaled, grad_scaled, n_comp, "scratch//results//cca//allTasks", save=True
)
...
x_weights_df.to_csv(
    f"scratch\\results\\cca\\allTasks\\pca_weights_noRot_{n_comp}ncomp.csv"
)
...
y_weights_df.to_csv(
    f"scratch\\results\\cca\\allTasks\\grad_weights_noRot_{n_comp}ncomp.csv"
)
