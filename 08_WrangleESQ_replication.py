...
esq_df = lab_data.drop(["Runtime_mod"
                  ], axis=1)
...
id_numbers_less_than_11_observations = observation_counts[observation_counts['Observation_count'] < 11]
...
grad_avg = long_grad_df.groupby(['Id_number', 'Task_name'], sort=False, as_index=False).mean()
...
grad_avg.to_csv("scratch//data//gradscores_spearman_combinedmask_cortical_subcortical_forCCA_prediction.csv", index = False)
