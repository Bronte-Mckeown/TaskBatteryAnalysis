...
from utils.cca import prepare_X, prepare_Y, train_CCA, test_CCA, prepare_variates, save_variates, heldout_pca
...
for (train, test) in cv.split(esq_data,grad_data,Taskindices):
    ...
    # 2. train PCA
    print ("about to PCA")
    output_train, output_test = heldout_pca(esq_data,esq_train,test_task, train, test,
                                             pca_n_comp, rotation = False, save = False)
    ...
    # 6. Train CCA
    ca, X_c_train, Y_c_train, X_Y_sum_train, X_c_scaler, Y_c_scaler, corrs = train_CCA(pca_train_scaled, grad_train_scaled, ca_n_comp, save=False)

    # 7. Test CCA
    X_c_test, Y_c_test, X_Y_sum_test = test_CCA(pca_test_scaled, grad_test_scaled, ca, X_c_scaler, Y_c_scaler)

    # 8. Prepare, concat, and save variates
    train_variates = prepare_variates(pca_train_scaled, X_c_train, Y_c_train, X_Y_sum_train)
