...
This script runs overall PCA and CCA on task battery data (all 14 tasks, N obs = 7220)

It then uses this overall CCA to predict Y values for tasks in task list (new task data).
...
from utils.cca import prepare_X, prepare_Y, train_CCA, tasklist_prediction, otherTask_pred_dist, heatmap_allDim, heatmap_perDim, ttests, print_ttests, select_cols
...
# So in the og data, as there is 2-3 obs per task, there are 38 obs per PS
...
# calculate 5 component solution on og mDES data (7220 obs)
n_comp = 5

model = basePCA(n_components=n_comp, rotation = False)
output = model.fit_transform(esq_data) # output is same as esq_data + 4 cols for PCA scores
...
df.to_csv("scratch//results//cca//allTasks//reptasks_cca_predictgrads.csv", index = False)
...
for task in true_task_distances_allDim.keys():
    ...
    df1.to_csv(f'scratch//results//cca//allTasks//{task}_dists_for_bargraphs.csv', index=False)
...
heatmap_allDim(len(task_list), custom_order, true_task_distances_allDim, otherTask_pred_dist_allDim, 
               "Predicted", "True",
                "otherPred_noRotation","scratch//results//cca//allTasks")

## EACH gradient separately (not used, just for checking)
# Create a separate heatmap for distances along each gradient
heatmap_perDim(len(task_list), custom_order, n_grads, true_task_distances_perDim, otherTask_pred_dist_perDim, 
               "Predicted", "True",
               "Distance between True and Predicted",
               "otherPred", "scratch//results//cca//allTasks")
