...
def project_gradCCA(cca_values, gradients, title, output, niftioutput):
    ...
    plotting.plot_glass_brain(ccaimage, plot_abs=False, colorbar=True, 
                              output_file=output, 
                              title=title)
...
cca_weights = pd.read_csv('scratch\\results\\cca\\allTasks\\grad_weights_noRot_5ncomp.csv')
ncomp = 5
...
for cca_dim in range(ncomp):
    ...
    project_gradCCA(cca_values,  gradientimgs, f"Thought-Brain Dimension {cca_dim+1}",
